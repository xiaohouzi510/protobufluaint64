        ...
        local list_field = function(fields)
            ...
            local iter = function(a, i)
                while true do
                    ...
                    if not _IsPresent(descriptor, value) then
                        i = descriptor
                    else 
...
function _AddGetAllFieldMethod(message_descriptor, message_meta)
    message_meta._member.GetAllField = function (self)
        local singular_fields = {}
        -- print("GetAllField, #self._fields: " .. #self._fields)
        for field, value in pairs(self._fields) do
            if field.label ~= FieldDescriptor.LABEL_REPEATED then
                singular_fields[field.full_name] = value._is_present_in_parent
            end
        end

        return singular_fields;
    end
end
...
local function _AddMessageMethods(message_descriptor, message_meta)
    ...
    _AddGetAllFieldMethod(message_descriptor, message_meta)
